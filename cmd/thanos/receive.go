...
func registerReceive(app *extkingpin.App) {
	...
	cmd.Setup(func(g *run.Group, logger log.Logger, reg *prometheus.Registry, tracer opentracing.Tracer, _ <-chan struct{}, _ bool) error {
		...
		// Determine which 'mode' this instance of Thanos receive is configured to run in.
		var receiveMode receive.ReceiverMode
		switch {
		case hashringSpecified && localEndpointSpecified:
			receiveMode = receive.RouterIngestor
		case hashringSpecified && !localEndpointSpecified:
			receiveMode = receive.RouterOnly
		case !hashringSpecified && !localEndpointSpecified:
			receiveMode = receive.IngestorOnly
		default:
			return errors.New("failed to determine receiver mode")
		}
...
func runReceive(
	...
	level.Info(logger).Log("msg", fmt.Sprintf("running receive in %s mode", receiveMode))
