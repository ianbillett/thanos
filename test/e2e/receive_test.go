...
func TestReceive(t *testing.T) {
	...
	t.Run("singe_router_multiple_ingester_replicated", func(t *testing.T) {
		/*
		The single_router_multiple_ingester_replicated suite configures separate routing and ingesting components.
		...
		*/
		...
		t.Parallel()
		s, err := e2e.NewScenario("e2e_receive_singe_router_multiple_ingester")
		testutil.Ok(t, err)
		t.Cleanup(e2ethanos.CleanScenario(t, s))
	...
	t.Run("multiple_routers_multiple_ingesters_replicated", func(t *testing.T) {
		/*
			The multiple_router_multiple_ingester_replicated suite configures a valid and plausible, but non-trivial
			topology of receiver components. Crucially, the first router routes to both a routing component, and a receiving
			component. This demonstrates Receiver's ability to handle arbitrary depth receiving trees.
		...
		*/
		...
		t.Parallel()
		s, err := e2e.NewScenario("e2e_receive_multiple_routers_multiple_ingesters_replicated")
		testutil.Ok(t, err)
		t.Cleanup(e2ethanos.CleanScenario(t, s))
